...
class Launchkey {
  ...
  Launchkey(PApplet parent) {
    ...
    buttoncolor("roundTop", 0, 0);
    buttoncolor("roundBottom", 0, 0);
  }
  ...
  void incontrol(boolean enable) {
    ...
    // sending note 12 also seems to work; not clear what the difference is
    control.sendNoteOn(0, 10, enable ? 127 : 0);
  }
  ...
  void buttoncolor(String buttonName, float red, float green) {
    int note;
    if (buttonName.equals("roundTop")) note = 104;
    else if (buttonName.equals("roundBottom")) note = 120;
    else return;

    if (debug) println("Setting button", buttonName, "red", red, "green", green);
    int col = int(red * 7) + (int(green * 7) << 4);
    control.sendNoteOn(0, note, col);
  }
}
